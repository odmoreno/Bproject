...
const promises = [
    ...
    d3.json("../../../data/periodo2.json"),
    d3.json("../../../data/datap2a.json")
]
...
function update(value) {
    ...
    console.log('sesion', sesiones[value])
    let nodosSesion =  sesiones[value].nodos
    let linksSesion =  sesiones[value].links
    //let leftnodes = sesiones[value].left
    let visitado = sesiones[value].visitado
    ...
    for (let i=0; i<nodosSesion.length; i++){
        ...
        else console.log('No existe en nodos in', id)
    }

    //for (let i=0; i<leftnodes.length; i++){
    //    id = leftnodes[i].id
    //    if(asambleistas[id]){
    //        asambleistas[id].visitado = false
    //    }
    //    else console.log('No existe en left', id)
    //}
    ...
    if (visitado == false) {
        links = [...links, ...linksSesion]
        sesiones[value].visitado = true
    }

    let linkstmp = links.map(d => Object.create(d));
    console.log('nodos asam', newnodes)
    console.log('links nuevos', linksSesion)
    ...
    validLinks = validateLinks(newnodes, linkstmp)
    nodes = nodes
    ...
    $("#sesion")[0].innerHTML = sesiones[value].sesion
    $("#votacion")[0].innerHTML = sesiones[value].votacion
    $("#asunto")[0].innerHTML = sesiones[value].asunto
}
...
var allGroups = [...new Set(partidos)]
var color1 = d3.scaleOrdinal().domain(allGroups).range(d3.schemePaired )
