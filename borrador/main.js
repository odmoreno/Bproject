...
const searchBarAsamb = document.getElementById('search-asamb');
...
function searchAsambleistas (searchText){
    console.log('asmb search', asambleistas)
    let ses = Object.values(asambleistas)
    let matches = ses.filter(per => {
        const regex =  new RegExp(`^${searchText}`, 'gi')
...
function outputAsambleistas (matches){
    if (matches.length > 0){
        const html = matches.map(match => 
          `
          <a href="#"  id=${match.numeroId}
          class="list-group-item list-group-item-action mb-1" data-toggle="list" role="tab" onclick="getAsmbId(this.id)">
            <p>${match.nombre} 
...
function getAsmbId (id){
    let asamb = asambleistas[id]
    console.log("id", id)
    console.log("asmb: ", asamb)
    searchBarAsamb.value = asamb.nombre
    matchList.innerHTML = '';
    //update(sesion)
